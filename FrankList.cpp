...
template <typename T>
FrankList<T>::FrankList(std::size_t size)
    : head(nullptr)
    , tail(nullptr)
    , ahead(nullptr)
    , atail(nullptr)
...
template <typename T>
FrankList<T>::FrankList(std::size_t size, const T& init)
    : head(nullptr)
    , tail(nullptr)
    , ahead(nullptr)
    , atail(nullptr)
...
template <typename T>
void FrankList<T>::resize(std::size_t s, const T& init)
{
...
template <typename T>
void FrankList<T>::push_front(const T& elem)
{
...
template <typename T>
void FrankList<T>::push_back(const T& elem)
{
    Node* tmp = new Node(elem);
    if (!head) {
        head = tmp;
...
bool FrankList<T>::operator<(const FrankList<T>& rhv) const
{
    std::size_t s1 = this->size();
    std::size_t s2 = rhv.size();
    if (s1 < s2) {
        return true;
    }
    if (s1 == s2) {
...
iter FrankList<T>::insert(iter pos, std::size_t s, const T& val)
{
...
template <typename T>
std::size_t FrankList<T>::remove(const T& val)
{
    std::size_t count = 0;
    iterator it = begin();
    while (it != end()) {
...
std::size_t FrankList<T>::remove_if(unary_predicate func)
{
    std::size_t count = 0;
    iterator it = begin();
    while (it != end()) {
...
template <typename T>
typename FrankList<T>::iterator FrankList<T>::find(const T& elem)
{
    iterator it = begin();
...
template <typename T>
typename FrankList<T>::iterator FrankList<T>::rfind(const T& elem)
{
...
template <typename T>
template <typename iter>
iter FrankList<T>::insert_def(iter pos, const T& val)
{
...
template <typename T>
template <typename iter>
iter FrankList<T>::insert_rev(iter pos, const T& val)
{
    assert(pos.ptr != nullptr);
    insert_def(pos, val);
    ++pos;
...
template <typename T>
T& FrankList<T>::iterator::operator*()
{
    return this->ptr->val;
}
...
template <typename T>
const typename FrankList<T>::const_multi_iterator FrankList<T>::const_multi_iterator::operator++(T)
{
...
template <typename T>
const typename FrankList<T>::const_multi_reverse_iterator FrankList<T>::const_multi_reverse_iterator::operator++(T)
{
